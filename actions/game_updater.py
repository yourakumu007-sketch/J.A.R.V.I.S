...
def _schedule_linux(hour: int, minute: int) -> str:
    script_path = Path(__file__).resolve()
    marker      = "# J.A.R.V.I.S_GameUpdater"
...
def _cancel_scheduled_update() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Delete", "/TN", "J.A.R.V.I.S_GameUpdater", "/F"],
            capture_output=True, text=True, **_CNW
        )
    ...
    try:
        existing = subprocess.run(["crontab", "-l"], capture_output=True, text=True)
        lines    = [l for l in existing.stdout.splitlines()
                    if "J.A.R.V.I.S_GameUpdater" not in l]
...
def _get_schedule_status() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Query", "/TN", "J.A.R.V.I.S_GameUpdater", "/FO", "LIST"],
            capture_output=True, text=True, **_CNW
        )
        if result.returncode != 0:
    ...
    try:
        ...
        if "J.A.R.V.I.S_GameUpdater" in result.stdout:
            for line in result.stdout.splitlines():
                if "J.A.R.V.I.S_GameUpdater" in line:
