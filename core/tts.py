"""
Text-to-Speech engines for HUNNY.
...
class EdgeTTSEngine:
    ...
    async def _synth(self, text: str) -> bytes:
        import edge_tts
        comm = edge_tts.Communicate(text, self.voice, pitch="-45Hz", rate="-18%")
        buf  = bytearray()
...
class PiperTTSEngine:
    """Piper offline neural TTS engine with EdgeTTS fallback."""

    def __init__(self, model_path: str = "en_US-ryan-high.onnx"):
    ...
    def speak(self, text: str) -> None:
        try:
            ...
            wav_file.close()
            cmd = ["piper", "--model", self.model_path, "--output_file", wav_file.name]
...
def create_tts_player(config: dict) -> TTSPlayer:
    ...
    if engine_name == "piper":
        engine = PiperTTSEngine(model_path=config.get("piper_model", "en_US-ryan-high.onnx"))
    elif engine_name == "kokoro":
        voice  = config.get("tts_voice", "bm_george")
